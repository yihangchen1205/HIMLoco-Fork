...
from mujoco_playground._src.locomotion.bk import bk_constants
from mujoco_playground._src.locomotion.bk.base import get_assets
...
class PyTorchControllerWithJoystick:
    ...
    def __init__(
        ...
        # 控制命令 - 初始为静止状态
        self.command = np.array([0.0, 0.0, 0.0])  # [x_vel, y_vel, angular_vel]
        self.raw_command = np.array([0.0, 0.0, 0.0])  # 原始未平滑的命令
    ...
    def _update_joystick_command(self):
        ...
        try:
            ...
            # 目标命令（按训练幅值缩放到物理单位）
            target_cmd = np.array([
                self.raw_command[0] * 1.5,
                self.raw_command[1] * 0.8,
                self.raw_command[2] * 1.2,
            ], dtype=np.float32)
            ...
            # 截断命令到训练时范围 [x_max=1.5, y_max=0.8, angular_max=1.2]
            self.command[0] = np.clip(self.command[0], -1.5, 1.5)
            self.command[1] = np.clip(self.command[1], -0.8, 0.8)
            self.command[2] = np.clip(self.command[2], -1.2, 1.2)
    ...
    def get_obs(self, data: mujoco.MjData) -> np.ndarray:
        ...
        # 使用游戏手柄的控制命令
        # 训练时 BK 环境命令范围是 x±1.5, y±0.8, ω±1.2，并且不取负号
        command = np.array([
            self.command[0],
            self.command[1],
            self.command[2]
        ...
        if self._debug_counter % 50 == 0:  # 每50帧打印一次
            raw_cmd_scaled = np.array([
                self.raw_command[0] * 1.5,
                self.raw_command[1] * 0.8,
                self.raw_command[2] * 1.2
            ])
            print(f"原始命令(未平滑,已按训练幅值缩放): [{raw_cmd_scaled[0]:.3f}, {raw_cmd_scaled[1]:.3f}, {raw_cmd_scaled[2]:.3f}] "
                  f"-> 平滑+截断后: [{command[0]:.3f}, {command[1]:.3f}, {command[2]:.3f}] "
                  f"(范围: x±1.5, y±0.8, ω±1.2)")

        # 构建观测向量 (固定45维: 3+3+12+12+12+3=45)
        obs_list = [
            # linvel,  # 注释掉，与原代码保持一致
            gyro,                    # 3维
            gravity,                 # 3维
...
def load_callback(model=None, data=None):
    ...
    model = mujoco.MjModel.from_xml_path(
        bk_constants.FEET_ONLY_FLAT_TERRAIN_XML.as_posix(),
        assets=get_assets(),
    )
    # 可选择粗糙地形
    # model = mujoco.MjModel.from_xml_path(
    ...
    policy = PyTorchControllerWithJoystick(
        model=model,
        policy_path="model/policy.pt",  # PyTorch .pt模型文件
        default_angles=np.array(model.keyframe("stand").qpos[7:]),
...
if __name__ == "__main__":
    ...
    print("  命令截断: x方向±1.5, y方向±0.8, 角速度±1.2 (防止过度控制)")
